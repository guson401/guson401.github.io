...
const staticTranslations = {
  ko: {
    ...
    stackReasons: {
      "reason-kotlin": "ASR과 TTS 기능을 포함한 음성 기반 일기 앱에서는 응답 속도와 기기 자원 활용이 중요하다고 판단했습니다. 그래서 React Native나 Expo 같은 크로스플랫폼 방식보다 Android 네이티브 환경에 더 직접적으로 접근할 수 있는 Kotlin을 선택했고, 이를 통해 음성 처리 라이브러리 연동과 화면 성능을 보다 안정적으로 구성할 수 있었습니다.",
      "reason-compose": "오브젝트 중심의 커스텀 UI를 빠르게 구성하고, 화면 상태 변화에 따라 로그인, 홈, 일기, 캘린더, 프로필 흐름을 일관되게 관리하기 위해 사용했습니다.",
      "reason-material3": "Compose 기반 UI에서 기본 컴포넌트와 상호작용 패턴을 안정적으로 활용하면서, 프로젝트 톤에 맞게 커스터마이징하기 위해 사용했습니다.",
      "reason-activity-compose": "Android Activity 진입점에서 Compose 화면을 자연스럽게 연결하고 앱의 시작 구조를 간결하게 유지하기 위해 사용했습니다.",
      "reason-lifecycle-compose": "화면 상태와 라이프사이클 변화를 Compose 화면 안에서 안정적으로 반영해 화면 전환과 상태 관리를 자연스럽게 처리하기 위해 사용했습니다.",
      "reason-room": "일기와 사용자 정보를 로컬에 안정적으로 저장하고, 화면 코드가 직접 데이터 저장소를 다루지 않도록 Repository 계층과 함께 구조를 분리하기 위해 사용했습니다.",
      "reason-repository": "화면과 데이터 소스를 분리해 네트워크, 로컬 DB, 기기 기능을 한 계층에서 정리하고 화면 코드의 결합도를 낮추기 위해 적용했습니다.",
      "reason-serialization": "앱 내부에서 사용하는 데이터 구조를 직렬화하고 상태나 응답 값을 Kotlin 코드 중심으로 다루기 위해 사용했습니다.",
      "reason-jackson": "서버 응답 데이터를 Kotlin 객체로 안정적으로 변환하고 Retrofit과 함께 API 응답 처리를 구성하기 위해 사용했습니다.",
      "reason-retrofit": "서버와의 API 통신을 인터페이스 기반으로 명확하게 분리하고, 응답 처리 구조를 일관되게 관리하기 위해 사용했습니다.",
      "reason-okhttp": "네트워크 요청과 응답 흐름을 안정적으로 처리하고, 인터셉터를 통해 통신 과정을 확장할 수 있도록 구성하기 위해 사용했습니다.",
      "reason-logging": "API 요청과 응답 로그를 확인해 서버 연동 과정에서 발생하는 문제를 빠르게 점검하고 디버깅하기 위해 사용했습니다.",
      "reason-security": "앱에서 다루는 민감한 정보와 인증 관련 데이터를 더 안전하게 저장하기 위한 보안 계층으로 활용했습니다.",
      "reason-calendar-provider": "감정 기록과 실제 기기 일정을 연결해 사용자가 앱 내부 기록과 일정 관리를 한 화면 흐름 안에서 함께 확인할 수 있도록 하기 위해 사용했습니다.",
      "reason-workmanager": "백그라운드 작업이나 지연 실행이 필요한 기능을 Android 환경에 맞게 안정적으로 처리하기 위해 사용했습니다.",
      "reason-sherpa": "앱 내 음성 처리 기능을 지원하기 위한 라이브러리로 활용해 음성 기반 입력 흐름을 구현할 수 있도록 구성했습니다.",
    },
